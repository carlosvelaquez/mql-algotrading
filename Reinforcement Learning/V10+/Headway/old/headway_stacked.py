...
class Headway(gym.Env):
    def __init__(self, history_path="./history", obs_window=5000, min_target_pts=25, max_target_pts=250, max_loss=200, gamma=1, norm_obs=True, debug=1):
        # Parameters
        ...
        self.observation_space = gym.spaces.Box(
            low=np.tile([0.75, 0.75, 0, 0.75, 0.75, 0.75, 0.75,
                         0.75, 0.75, 0.75, 0.75, 0, 0, 0, 0.75, 0.75, 0.75], (obs_window, 1)),
            high=np.tile([1.75, 1.75, 250, 1.75, 1.75, 1.75, 1.75,
                          1.75, 1.75, 1.75, 1.75, 100, 100, 100, 1.75, 1.75, 1.75], (obs_window, 1))
        )
    ...
    def load_history(self):
        ...
        if self.history_size < self.obs_window:
            self.load_history()

    def get_obs(self):
        obs = self.history[self.current_pos -
                           self.obs_window: self.current_pos]
        self.current_pos += 1
    ...
    def reset(self):
        # Trackers
        self.current_pos = self.obs_window
